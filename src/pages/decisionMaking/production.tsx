...
import { getProductionDecision } from './service';
...
const getTopoGraph = (data: any) => {
  ...
  let skillNum = 0; // 记录所有skill数目作为links的增幅
  let skillCount = 0; // 记录单次遍历中已经遍历过的skill数目
  const graph: any = {
    nodes: [
      {
        id: "0",
  ...
  for (let i = 0; i < corporationList.length; i++) {
    for (let j = 0; j < data[corporationList[i]].length; j++) {
      ...
      graph.links.push({
        source: `${corporationList.length + 1 + skillCount}`,
        target: `${corporationList.length + 1 + skillCount + skillNum}`,
        relation: { name: '疑似根因' },
      });
      skillCount++;
    }
  ...
  for (let i = 0; i < corporationList.length; i++) {
    for (let j = 0; j < data[corporationList[i]].length; j++) {
      ...
      graph.links.push({
        source: `${corporationList.length + 1 + skillCount + skillNum}`,
        target: `${corporationList.length + 1 + skillCount + 2 * skillNum}`,
        relation: { name: '决策措施' },
      });
      skillCount++;
    }
...
const production: FC = (): ReactElement => {
  ...
  const onFinish = (values: any) => {
    ...
    getProductionDecision(params).then((res: any) => {
      // table根据当前表单公司来展示对应公司数据
      const currentDataSource = getCurrentDataSource(res.data)
      setDataSource(currentDataSource);
      // 填充圆环图数据
      const corporationList = getCorporationList(res.data);
